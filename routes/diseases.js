/*
    ...
    POST /app/admin/adddisease -> add a new disease in the system
...
/*
    POST /app/admin/adddisease -> add a new disease in the system
*/
router.post('/app/admin/adddisease', (req, res) => {
    ...
    if (_.isString(diseaseName) && !_.isNaN(diseaseScore)) {
        ...
        disease.save().then((disease) => {
            ...
            res.status(200).redirect('/app/admin/admindisease');
        }).catch((err) => {
            console.log(err);
            res.status(400).redirect('/app/admin/admindisease');
        });
    } else {
        res.status(400).redirect('/app/admin/admindisease',{messages: req.flash('success_msg', 'Succesful test') });
    }
});
...
/*
    POST /app/deletedisease -> delete a disease from the system
*/
router.post('/app/admin/deletediseases', (req, res) => {
    ...
    if (_.isArray(diseasesToDelete)) {
        ...
        console.log("POST /app/admin/deletedisease, diseasesToDelete is not an array");
        res.status(400).redirect('/app/admin/adminndisease');
    }
});
