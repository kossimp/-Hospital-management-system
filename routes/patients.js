/*
     GET /app/user/addpatient -> go to addPatient page
     POST /app/user/addpatient                -> add a patient in the database
...
/*
    GET /app/user/addpatient -> go to addPatient page
*/
router.get('/app/user/addpatient', (req, res) => {
    res.render('user/addpatient', {pageTitle: "添加病人",username:req.user.username});
});

/*
    POST /addPatient -> add new patient
*/
router.post('/app/user/addpatient', (req, res) => {
    ...
    if (_.isEmpty(req.body.firstName) || _.isEmpty(req.body.lastName) || _.isEmpty(req.body.hospitalNumber) || !isValidDate(dateOfBirth)) {
        ...
        res.status(400).redirect('/user/addpatient');
    } else {
...
router.get('/app/patient/:hospitalNumber', (req, res) => {
    ...
    Patient.findOne({
        ...
        res.status(200).render('user/patientPage',{username:req.user.username});
    }).catch((err) => {
        console.log(err);
...
/*
    POST /app/updatepatient/:hospitalNumber -> update disease & score for patient
                                            -> request made from the user/patientPage
*/
